    ...
    <div className="absolute inset-0 flex items-center justify-center">
      <div className="grid grid-cols-2 gap-3 p-6">
        {['Dashboard', 'LINE', 'AI', 'Sheet'].map((t) => (
          <div
            key={t}
            className="card-float px-5 py-4 text-sm font-semibold text-cyan-700"
          >
            {t}
          </div>
        ))}
    ...
    <div className={className}>
      ...
      >
        <Suspense fallback={null}>
          ...
          <pointLight position={[0, 3, 2]} intensity={0.6} color="#22d3ee" distance={15} />
          <pointLight position={[-4, -2, 3]} intensity={0.4} color="#2dd4bf" distance={12} />
