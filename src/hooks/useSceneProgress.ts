...
const sceneEnvExposure: Record<SceneId, number> = {
  hero: 0.45,
  chaos: 0.5,
  lightOn: 0.85,
  organize: 0.75,
...
export function useSceneScrollTriggers() {
  ...
  const setEnvExposure = useSceneStore((s) => s.setEnvExposure)
    ...
    sections.forEach((section) => {
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          setEnvExposure(sceneEnvExposure[scene])
        },
        onEnterBack: () => {
          setCurrentScene(scene)
          setLightIntensity(sceneIntensity[scene])
      ...
      if (scene === 'hero') {
        ScrollTrigger.create({
          ...
          scrub: true,
          onUpdate: (self) => {
            setLightIntensity(0.05 + self.progress * 0.35)
            setEnvExposure(0.4 + self.progress * 0.35)
          },
        })
      }
